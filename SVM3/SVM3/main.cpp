...
void trainSVM(vector<double> &x1, vector<double> &x2, vector<int> &y, double &w1, double &w2, double &b) {
    ...
    while (true) {
        ...
        if (cost == prev_cost) {
            no_change_iter++;
            if (no_change_iter >= 10000) {
                cout << "Cost hasn't changed for 10,000 iterations, stopping early." << endl;
                break;
            }
        } else {
            no_change_iter = 0;
...
void readData(const string &filename, vector<double> &x1, vector<double> &x2, vector<int> &y, int label, int max_points = 10) {
...
int main() {
    ...
    readData("C:\\Users\\Jyo\\Downloads\\OneDrive_1_14-06-2024\\u.txt", X1, X2, Y, 1);
